...
class QueryPropertiesTest extends FunSuite with BeforeAndAfter with ScalaFutures {
  ...
  after {
    ...
    dals.loggingDal.deleteQueryName(QUERY_NAME_1, "testUser")
    dals.loggingDal.deleteQueryName(QUERY_NAME_2, "testUser")
  }

  def createQuery(queryId:String, queryScript:String): Unit = {
    val metaInfo = new QueryMetaInfo()
    dals.loggingDal.setMetaInfo(queryId, metaInfo, "testUser")
    dals.loggingDal.setState(queryId, QueryState.IN_PROGRESS, "testUser")
    dals.loggingDal.setScriptDetails(queryId, queryScript, "testUser")
  }
  ...
  test(" not found ") {
    ...
    val f = tAct ? UpdateQueryPropertiesMessage(queryId, Some(""), Some(""), None, overwrite = false, "testUser")
  ...
  test ("When setting the name the query should be updated") {
    val f = tAct ? UpdateQueryPropertiesMessage(queryId, Some(QUERY_NAME_1), Some(""), None, overwrite = false, "testUser")
  ...
  test ("When setting the name the query and searching for it, the query should be returned") {
    ...
    whenReady(f, timeout) (s => {
      ...
      val query = dals.loggingDal.getQueriesByName(QUERY_NAME_1, "testUser").queries(0)
  ...
  test ("Updating the query name twice should delete the old name") {
    ...
    val f2 = tAct ? UpdateQueryPropertiesMessage(queryId, Some(QUERY_NAME_2), Some(""), None, overwrite = false, "testUser")
    whenReady(f2) (_ => {
      assert(dals.loggingDal.getQueriesByName(QUERY_NAME_1, "testUser").queries.length === 0, "The first query name should not be found")
      val query = dals.loggingDal.getQueriesByName(QUERY_NAME_2, "testUser").queries(0)
      assert(query.query === queryScript)
      assert(query.queryID === queryId, "The query id is not the expected one")
  ...
  test ("Setting a name for the queries that is not unique should throw exception when overwrite is false") {
    ...
    val f = tAct ? UpdateQueryPropertiesMessage(queryId, Some(QUERY_NAME_1), Some(""), None, overwrite = false, "testUser")
    Await.ready(f, 10.seconds)
    val f2 = tAct ? UpdateQueryPropertiesMessage(secondQueryId, Some(QUERY_NAME_1), Some(""), None, overwrite = false, "testUser")

    whenReady(f2, timeout) (s => {
  ...
  test ("Setting a name for the queries that is not unique should be updated when overwrite is true") {
    ...
    val f = tAct ? UpdateQueryPropertiesMessage(queryId, Some(QUERY_NAME_1), Some(""), None, overwrite = false, "testUser")
    Await.ready(f, 10.seconds)
    val f2 = tAct ? UpdateQueryPropertiesMessage(secondQueryId, Some(QUERY_NAME_1), Some(""), None, overwrite = true, "testUser")
  ...
  test ("Setting a query that has a name with null should delete its name") {
    val f = tAct ? UpdateQueryPropertiesMessage(queryId, Some(QUERY_NAME_1), Some(""), None, overwrite = false, "testUser")
    Await.ready(f, 10.seconds)
    val f2 = tAct ? UpdateQueryPropertiesMessage(queryId, Some(null), Some(""), None, overwrite = true, "testUser")

    whenReady(f2, timeout) (s => {
      val newMetaInfo = dals.loggingDal.getMetaInfo(queryId, "testUser")
      assert(newMetaInfo.name === None)
      assert(dals.loggingDal.getQueriesByName(QUERY_NAME_1, "testUser").queries.length === 0, "The name of the query should be deleted")
      assert(s === s"Query information for $queryId has been updated")
    })
  }

  test ("Setting a query published should keep the flag") {
    val f = tAct ? UpdateQueryPropertiesMessage(queryId, Some(QUERY_NAME_1), Some(""), Some(true), overwrite = false, "testUser")
    whenReady(f, timeout) (s => {
      val newMetaInfo = dals.loggingDal.getMetaInfo(queryId, "testUser")
      assert(newMetaInfo.published === Some(true), "The flag is not saved")

      val publishedQueries = dals.loggingDal.getPublishedQueries("testUser")
  ...
  test ("Renaming a published query should keep the flag") {
    val f = tAct ? UpdateQueryPropertiesMessage(queryId, Some(QUERY_NAME_1), Some(""), Some(true), overwrite = false, "testUser")
    Await.ready(f, 10.seconds)
    val f2 = tAct ? UpdateQueryPropertiesMessage(queryId, Some(QUERY_NAME_2), Some(""), None, overwrite = true, "testUser")
  ...
  test ("Unpublishing a query should set the flag on false") {
    val f = tAct ? UpdateQueryPropertiesMessage(queryId, Some(QUERY_NAME_1), Some(""), Some(true), overwrite = false, "testUser")
    Await.ready(f, 10.seconds)
    val f2 = tAct ? UpdateQueryPropertiesMessage(queryId, None, None, Some(false), overwrite = true, "testUser")
